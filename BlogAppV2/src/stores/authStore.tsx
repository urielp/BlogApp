...
const authStore = () =>{
    ...
    const actions = {
        LOGIN:async (curState:any,payload:any) =>{
          let promiseState =   axios.post('http://localhost:3000/users/login',payload,
            {
                headers:{
                ...
                return {isLoggedIn:returnedResults.data.isLoggedIn,
       ...
       IS_AUTHONTICATED:async (curState:any,payload:any) =>{
        let promiseState = await axios.get('http://localhost:3000/users/verifytoken',
            {
                headers:{
                Authorization:payload.token
                } 
